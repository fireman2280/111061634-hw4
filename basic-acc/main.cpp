...
public:
	...
	addr_t medfilter_start_addr = 0x73000000;
 	addr_t medfilter_size = 0x01000000;
 	addr_t medfilter_end_addr = medfilter_start_addr + medfilter_size - 1;
	...
	void parse(int argc, char **argv) override {
		...
		assert((mem_end_addr < clint_start_addr || mem_start_addr > medfilter_end_addr) &&
...
int sc_main(int argc, char **argv) {
	...
	medfilter med_filter("med_filter");
	...
	bus.ports[12] = new PortMapping(opt.medfilter_start_addr, opt.medfilter_end_addr);
	...
	bus.isocks[12].bind(med_filter.tsock);
